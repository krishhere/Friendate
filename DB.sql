...
DELIMITER $$;
Create PROCEDURE Pro_UserEmail_Insert(in p_Email varchar(40),in p_Password varchar(12))
BEGIN
insert into users(Email,Password) values(p_Email,p_Password);
END $$;

DELIMITER $$;
Create PROCEDURE Pro_User_Insert(in p_name varchar(12),in p_city char(15),in p_dob varchar(12),in p_gender tinyint,in p_lookFor tinyint,in p_about varchar(300),in p_image1 mediumblob,in p_id bigint)
BEGIN
update users set Name=p_name,city=p_city,dob=p_dob,gender=p_gender,lookFor=p_lookFor,about=p_about,image1=p_image1 where id=p_id;
END $$;
